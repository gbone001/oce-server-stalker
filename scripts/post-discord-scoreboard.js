...
async function loadServersConfig() {
  ...
  return data.servers.map((entry, index) => {
    if (!entry || typeof entry !== 'object') {
...
function mapApiResponse(data, server) {
  ...
  const totalPlayers =
    num(data?.player_count, undefined) ??
    num(data?.players_total, undefined) ??
  ...
  const axisScore =
    num(data?.axisScore, undefined) ??
    num(data?.score?.axis, undefined) ??
    num(data?.axis_score, undefined) ??
    0;
  ...
  return {
    ...
    currentMap,
    nextMap,
    timeRemainingSeconds: toTime(
      num(data?.time_remaining, undefined) ?? num(data?.remaining_time, undefined)
    ),
  };
}
...
function buildDiscordMessage(statuses) {
  ...
  const lines = statuses.map((status) => {
    ...
    const total = typeof status.totalPlayers === 'number' ? status.totalPlayers : status.alliesPlayers + status.axisPlayers;
